...
[[nodiscard]] bool isScalarObject(const SemanticSymbol* symbol)
{
    return symbol != nullptr && symbol->isObject() && symbol->object().m_type.isScalar();
}
...
[[nodiscard]] Ptr<Exp> scalarInitExp(const AST& ast, Ref<ConstDef> def)
{
    ...
    const auto* exp = std::get_if<Ref<Exp>>(&def(ast).constInitVal(ast).kind);
    ...
    void walkExp(Ref<Exp> exp) const
    {
        MATCH(exp(m_ast).kind) WITH(
            ...
            [&](const Exp::LVal& lVal) {
                ...
                const auto symbolId = m_symbolResult.findSymbolId(lVal.identifier);
                if (!symbolId.has_value() || !m_localScalarSymbols.contains(*symbolId)) {
                    return;
                }
...
struct FunctionAnalyzer {
    ...
    std::unordered_set<int32_t> m_localScalarSymbols;
    std::unordered_map<int32_t, std::vector<Block>> m_defBlocksBySymbol;
    std::unordered_map<Block, std::unordered_set<Block>> m_dominatorsByBlock;
    ...
    [[nodiscard]] const SemanticSymbol* findSymbol(Ref<Identifier> identifier) const
    {
        const auto symbolId = m_symbolResult.findSymbolId(identifier);
        return symbolId.has_value() ? m_typeResult.findSymbolById(*symbolId) : nullptr;
    }

    void collectLocalScalarSymbols()
    {
        for (const auto& param : m_result.m_funcDef(m_ast).funcFParams) {
            const auto symbolId = m_symbolResult.findSymbolId(param.identifier);
            if (!symbolId.has_value() || !isScalarObject(findSymbol(param.identifier))) {
                continue;
            }
            m_localScalarSymbols.insert(*symbolId);
        }
                        ...
                        WITH(
                            [&](Ref<ConstDecl> constDecl) {
                                for (auto def : constDecl(m_ast).constDef) {
                                    const auto symbolId = m_symbolResult.findSymbolId(
                                        def(m_ast).identifier);
                                    if (!symbolId.has_value()
                                        || !isScalarObject(findSymbol(def(m_ast).identifier))) {
                                        continue;
                                    }
                                    m_localScalarSymbols.insert(*symbolId);
                                }
    ...
    void initializeEdges()
    {
        for (auto block : m_blocks) {
            ...
            MATCH(*block(m_cfgArena).terminator) WITH(
        ...
        for (auto block : m_blocks) {
            for (auto succ : m_result.m_blockInfoByBlock.at(block).m_successors) {
                m_result.m_blockInfoByBlock.at(succ).m_predecessors.push_back(block);
            }
        }
    }

    [[nodiscard]] std::unordered_set<Block> reachableWithout(Block removed) const
    {
    ...
    [[nodiscard]] bool dominates(Block lhs, Block rhs) const
    {
        ...
        if (lhsIt == m_dfsInByBlock.end() || rhsIt == m_dfsInByBlock.end()) {
            return false;
        }
    ...
    void computeDominators()
    {
        ...
        m_result.m_blockInfoByBlock.at(cfg->entryBlock).m_immediateDominator.reset();
        for (auto block : m_blocks) {
            ...
            std::sort(candidates.begin(), candidates.end(), [&](Block lhs, Block rhs) {
            ...
            for (auto candidate : candidates) {
                ...
                for (auto other : candidates) {
                    ...
                    if (!m_dominatorsByBlock.at(candidate).contains(other)) {
                        isImmediate = false;
                        break;
                    }
                }
                ...
                m_result.m_blockInfoByBlock.at(block).m_immediateDominator = candidate;
                m_result.m_blockInfoByBlock.at(candidate).m_dominatorTreeChildren.push_back(block);
                break;
            }
        }
        ...
        std::function<void(Block)> dfs = [&](Block block) {
            ...
            auto& children = m_result.m_blockInfoByBlock.at(block).m_dominatorTreeChildren;
            std::sort(children.begin(), children.end(), [&](Block lhs, Block rhs) {
                return m_blockOrder.at(lhs) < m_blockOrder.at(rhs);
            });
        ...
        for (auto x : m_blocks) {
            for (auto y : m_blocks) {
                ...
                for (auto pred : m_result.m_blockInfoByBlock.at(y).m_predecessors) {
                ...
                if (onFrontier) {
                    m_result.m_blockInfoByBlock.at(x).m_dominanceFrontier.push_back(y);
                }
            }
        }
    }
    ...
    void computeUseDefSets()
    {
        for (auto block : m_blocks) {
            ...
            const ExpressionWalker walker {
                ...
                .m_localScalarSymbols = m_localScalarSymbols,
                .m_onRead = onRead,
            };
                ...
                MATCH(item) WITH(
                    [&](Decl decl) {
                        ...
                        WITH(
                            [&](Ref<ConstDecl> constDecl) {
                                for (auto def : constDecl(m_ast).constDef) {
                                    const auto initExp = scalarInitExp(m_ast, def);
                                    if (initExp != nullptr) {
                                    ...
                                    const auto symbolId = m_symbolResult.findSymbolId(
                                        def(m_ast).identifier);
                                    if (symbolId.has_value()
                                        && m_localScalarSymbols.contains(*symbolId)) {
                    ...
                    [&](Ref<AssignStmt> assignStmt) {
                        const auto* lVal = std::get_if<Exp::LVal>(&assignStmt(m_ast).lval(m_ast).kind);
                        if (lVal != nullptr) {
                            for (auto index : lVal->indices) {
                        ...
                        const auto identifier = scalarLValIdentifier(m_ast, assignStmt(m_ast).lval);
            ...
            if (block(m_cfgArena).terminator.has_value()) {
                MATCH(*block(m_cfgArena).terminator) WITH(
                    ...
                    [&](const SemanticReturnTerminator& ret) {
                        ...
                        if (const auto* exp = std::get_if<Ref<Exp>>(&ret.value->kind)) {
                            walker.walkExp(*exp);
                        }
                    });
            }
        ...
        for (auto& [symbolId, blocks] : m_defBlocksBySymbol) {
            std::sort(blocks.begin(), blocks.end(), [&](Block lhs, Block rhs) {
                return m_blockOrder.at(lhs) < m_blockOrder.at(rhs);
            });
        ...
        while (changed) {
            ...
            for (auto it = m_blocks.rbegin(); it != m_blocks.rend(); ++it) {
                ...
                for (auto succ : info.m_successors) {
                    const auto& succLiveIn = m_result.m_blockInfoByBlock.at(succ).m_liveIn;
    ...
    {
        std::vector<int32_t> symbols(m_localScalarSymbols.begin(), m_localScalarSymbols.end());
        ...
        for (auto symbolId : symbols) {
            ...
            while (!worklist.empty()) {
                ...
                worklist.pop();
                for (auto frontier : m_result.m_blockInfoByBlock.at(block).m_dominanceFrontier) {
                    auto& frontierInfo = m_result.m_blockInfoByBlock.at(frontier);
                    if (!frontierInfo.m_liveIn.contains(symbolId)) {
                        continue;
                    }
                    const bool hasParam = std::any_of(frontierInfo.m_params.begin(),
                        frontierInfo.m_params.end(), [&](const auto& param) {
                            return param.m_symbolId == symbolId;
                        });
                    if (!hasParam) {
                        frontierInfo.m_params.push_back(SemanticSsaBlockParam { .m_symbolId = symbolId });
                    }
    ...
    {
        const auto onRead = [&](Ref<Identifier> identifier, int32_t symbolId) {
            const auto it = aliases.find(symbolId);
            if (it != aliases.end()) {
                m_aliasByIdentifier.insert_or_assign(identifier, it->second);
            }
        };
        const ExpressionWalker walker {
            .m_ast = m_ast,
    ...
    void assignAliases()
    {
        ...
        std::function<void(Block, const std::unordered_map<int32_t, SemanticSsaAlias>&)> dfs
            = [&](Block block,
                  const std::unordered_map<int32_t, SemanticSsaAlias>& inheritedAliases) {
                  ...
                  const auto defineIdentifier = [&](Ref<Identifier> identifier) {
                      const auto symbolId = m_symbolResult.findSymbolId(identifier);
                      if (!symbolId.has_value() || !m_localScalarSymbols.contains(*symbolId)) {
                          return;
                      }
                      ...
                      m_aliasByIdentifier.insert_or_assign(identifier, alias);
                  };
                  ...
                  for (const auto& item : block(m_cfgArena).items) {
                      MATCH(item) WITH(
                              ...
                              WITH(
                                  [&](Ref<ConstDecl> constDecl) {
                                      for (auto def : constDecl(m_ast).constDef) {
                                          const auto initExp = scalarInitExp(m_ast, def);
                                          if (initExp != nullptr) {
                                              bindScalarReads(initExp.ref(), aliases);
                                          }
                                          defineIdentifier(def(m_ast).identifier);
                                      }
                                  },
                                  [&](Ref<VarDecl> varDecl) {
                                      for (auto def : varDecl(m_ast).varDef) {
                          ...
                          [&](Ref<ExpStmt> expStmt) {
                              if (expStmt(m_ast).exp != nullptr) {
                                  bindScalarReads(expStmt(m_ast).exp.ref(), aliases);
                              }
                          });
                  }
                  ...
                  for (auto succ : blockInfo.m_successors) {
                      ...
                      for (const auto& param : m_result.m_blockInfoByBlock.at(succ).m_params) {
                          const auto it = aliases.find(param.m_symbolId);
                          if (it != aliases.end()) {
                              args.push_back(it->second);
                          }
                      }
                      blockInfo.m_outgoingArgsByTarget.insert_or_assign(succ, std::move(args));
                  }
    ...
    void analyze(Ref<CompUnit> compUnit)
    {
        ...
        for (const auto& topLevelItem : compUnit(ast).topLevelItems) {
            ...
            for (auto block : cfg->blocks) {
                functionSSA.m_blockInfoByBlock.emplace(block,
                    SemanticSsaBlockInfo { .m_block = block });
            }
            auto [it, inserted] = m_functionByFuncDef.emplace(*funcDef, std::move(functionSSA));
            (void)inserted;
