...
[[nodiscard]] bool isTrackedObject(const SemanticSymbol* symbol)
{
    return symbol != nullptr && symbol->isObject()
        && (symbol->object().m_type.isScalar() || symbol->object().m_type.isPoly());
}
    ...
    void walkExp(Ref<Exp> exp) const
    {
        MATCH(exp(m_ast).kind) WITH(
            ...
            [&](const Exp::LVal& lVal) {
                ...
                if (!symbolId.has_value() || !m_localTrackedSymbols.contains(*symbolId)) {
                    return;
                }
...
struct FunctionAnalyzer {
    ...
    void collectLocalScalarSymbols()
    {
        for (const auto& param : m_result.m_funcDef(m_ast).funcFParams) {
            ...
            if (!symbolId.has_value() || !isTrackedObject(findSymbol(param.identifier))) {
                continue;
            }
                            ...
                            [&](Ref<ConstDecl> constDecl) {
                                for (auto def : constDecl(m_ast).constDef) {
                                    ...
                                    if (!symbolId.has_value()
                                        || !isTrackedObject(findSymbol(def(m_ast).identifier))) {
                                        continue;
                                    }
                                    m_localTrackedSymbols.insert(*symbolId);
                                }
    ...
    {
        std::vector<int32_t> symbols(m_localTrackedSymbols.begin(), m_localTrackedSymbols.end());
