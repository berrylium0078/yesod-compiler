...
class FunctionEmitter {
  ...
  public:
    ...
    void emitFunction(const Function& function)
    {
        if (function.getNumParams() != 0) {
            throw std::runtime_error("RISC-V backend does not support function parameters yet");
        }
  ...
  private:
    ...
    {
        return value.isAllocValue() || value.isLoadValue() || value.isBinaryValue();
    }
    ...
    void loadValueToRegister(const Value& value, const std::string& targetRegister)
    {
    ...
    void emitBinary(const BinaryValue& binaryValue)
    {
        ...
        case KOOPA_RBO_SAR:
            ...
            break;
        default:
            throw std::runtime_error("unsupported Koopa binary op in RISC-V backend");
        }
