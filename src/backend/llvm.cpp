...
std::string emitType(const koopa_ir::Type& type,
    const koopa_ir::Program& program)
{
    return MATCH(type) WITH(
        [](const koopa_ir::I32Type&) -> std::string {
            return "i32";
        },
        ...
        [&](const yesod::Ref<koopa_ir::FunctionType>& funcRef) -> std::string {
...
std::string emitValueOperand(const koopa_ir::Value& value,
    const koopa_ir::Program& /*program*/)
{
    return MATCH(value) WITH(
        ...
        [](const koopa_ir::UndefValue&) -> std::string {
            return "undef";
        });
}
    ...
    MATCH(initializer) WITH(
        ...
        [&](const yesod::Ref<koopa_ir::AggregateInitializer>& aggRef) {
            ...
            for (size_t i = 0; i < agg.elements.size(); ++i) {
                ...
                emitInitializer(output, agg.elements[i], elementType, program);
            }
            output << "}";
        });
}
...
std::string pointeeTypeString(const koopa_ir::Type& type,
    const koopa_ir::Program& program)
{
    if (const auto* pref
...
using IncomingMap
    = std::unordered_map<std::string, std::vector<std::vector<IncomingEdge>>>;
    ...
    for (const auto& blockRef : function.blocks) {
        ...
        MATCH(block.terminator) WITH(
            [&](const yesod::Ref<koopa_ir::JumpTerminator>& jumpRef) {
                ...
                for (size_t i = 0; i < jump.args.size() && i < slots.size(); ++i) {
                    slots[i].push_back(IncomingEdge { predLabel, jump.args[i] });
                }
            },
            [&](const yesod::Ref<koopa_ir::BranchTerminator>& brRef) {
                const auto& br = program[brRef];
                {
                    ...
                    for (size_t i = 0; i < br.trueArgs.size() && i < tslots.size(); ++i) {
                        tslots[i].push_back(IncomingEdge { predLabel, br.trueArgs[i] });
                    }
                }
                {
                    const std::string flabel = br.falseTarget.spelling;
                    auto& fslots = incoming[flabel];
                    for (size_t i = 0; i < br.falseArgs.size() && i < fslots.size(); ++i) {
                        fslots[i].push_back(IncomingEdge { predLabel, br.falseArgs[i] });
                    }
                }
            },
    ...
    std::ostream& output)
{
    ...
    auto recordPointer = [&](const std::string& sym, const koopa_ir::Type& type) {
        valueTypes[sym] = "ptr";
        const std::string pt = pointeeTypeString(type, program);
        if (!pt.empty()) { pointees[sym] = pt; }
    };
        ...
        std::visit(
            [&](auto itemRef) {
                ...
                if constexpr (std::same_as<Item, koopa_ir::GlobalMemoryDef>) {
                    ...
                    pointees[g.name.spelling] = emitType(g.allocType, program);
                }
            },
            item);
    }
    ...
    for (const auto& blockRef : function.blocks) {
        ...
        for (const auto& stmt : block.statements) {
            std::visit(
                [&](auto stmtRef) {
                    using StmtNode = std::remove_cvref_t<decltype(program[stmtRef])>;
                    if constexpr (std::same_as<StmtNode, koopa_ir::SymbolDef>) {
                        ...
                        MATCH(sd.rhs) WITH(
                            [&](const yesod::Ref<koopa_ir::MemoryDeclaration>& memRef) {
                                ...
                                pointees[sname] = emitType(mem.allocType, program);
                            },
                            [&](const yesod::Ref<koopa_ir::LoadExpr>& loadRef) {
                                const auto& ld = program[loadRef];
                                const auto it = pointees.find(ld.source.spelling);
                                valueTypes[sname] = (it != pointees.end()) ? it->second : "i32";
                            },
                            [&](const yesod::Ref<koopa_ir::GetPointerExpr>& getptrRef) {
                                const auto& gp = program[getptrRef];
                                const auto vtIt = valueTypes.find(gp.source.spelling);
                                valueTypes[sname] = (vtIt != valueTypes.end()) ? vtIt->second : "ptr";
                                const auto ptIt = pointees.find(gp.source.spelling);
                                if (ptIt != pointees.end()) { pointees[sname] = ptIt->second; }
                            },
                            [&](const yesod::Ref<koopa_ir::GetElementPointerExpr>& gelemRef) {
                                const auto& ge = program[gelemRef];
                                valueTypes[sname] = "ptr";
                                const auto ptIt = pointees.find(ge.source.spelling);
                                if (ptIt != pointees.end()) {
                                    const std::string& arrType = ptIt->second;
                                    auto xpos = arrType.find(" x ");
                                    if (xpos != std::string::npos) {
                                        std::string elemTy = arrType.substr(xpos + 3);
                                        if (!elemTy.empty() && elemTy.back() == ']') {
                                            elemTy.pop_back();
                                        }
                            ...
                            [&](const yesod::Ref<koopa_ir::CallExpr>& callRef) {
                                ...
                                if (sigIt != sigs.end() && sigIt->second.retType != "void") {
                                    valueTypes[sname] = sigIt->second.retType;
                                } else {
                                    valueTypes[sname] = "i32";
                                }
                            });
    ...
    for (const auto& blockRef : function.blocks) {
                        ...
                        MATCH(sd.rhs) WITH(
                            [&](const yesod::Ref<koopa_ir::MemoryDeclaration>& memRef) {
                                ...
                                output << "  " << llvmValueName(sname)
                                       << " = alloca " << emitType(mem.allocType, program) << "\n";
                            },
                            [&](const yesod::Ref<koopa_ir::LoadExpr>& loadRef) {
                                const auto& ld = program[loadRef];
                                const auto ptIt = pointees.find(ld.source.spelling);
                                const std::string elemTy = (ptIt != pointees.end()) ? ptIt->second : "i32";
                                output << "  " << llvmValueName(sname)
                                       << " = load " << elemTy << ", ptr "
                            ...
                            [&](const yesod::Ref<koopa_ir::GetPointerExpr>& getptrRef) {
                                const auto& gp = program[getptrRef];
                                const auto ptIt = pointees.find(gp.source.spelling);
                                const std::string gepElemTy = (ptIt != pointees.end()) ? ptIt->second : "i32";
                                output << "  " << llvmValueName(sname)
                            ...
                            [&](const yesod::Ref<koopa_ir::GetElementPointerExpr>& gelemRef) {
                                const auto& ge = program[gelemRef];
                                const auto ptIt = pointees.find(ge.source.spelling);
                                const std::string gepTy = (ptIt != pointees.end()) ? ptIt->second : "[0 x i32]";
                                output << "  " << llvmValueName(sname)
                                       << " = getelementptr " << gepTy << ", ptr "
                                       << llvmValueName(ge.source.spelling) << ", i32 0, i32 "
                                       << emitValueOperand(ge.index, program) << "\n";
                            },
                            [&](const yesod::Ref<koopa_ir::BinaryExpr>& binRef) {
                                const auto& bin = program[binRef];
                                const std::string lhs_typed = "i32 " + emitValueOperand(bin.lhs, program);
                                const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                using BOp = koopa_ir::BinaryOp;
                                switch (bin.op) {
                                case BOp::add: output << "  " << llvmValueName(sname) << " = add " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::sub: output << "  " << llvmValueName(sname) << " = sub " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::mul: output << "  " << llvmValueName(sname) << " = mul " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::div: output << "  " << llvmValueName(sname) << " = sdiv " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::mod: output << "  " << llvmValueName(sname) << " = srem " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::bitAnd: output << "  " << llvmValueName(sname) << " = and " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::bitOr:  output << "  " << llvmValueName(sname) << " = or " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::bitXor: output << "  " << llvmValueName(sname) << " = xor " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::shl:    output << "  " << llvmValueName(sname) << " = shl " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::shr:    output << "  " << llvmValueName(sname) << " = lshr " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::sar:    output << "  " << llvmValueName(sname) << " = ashr " << lhs_typed << ", " << rhs_bare << "\n"; break;
                                case BOp::eq: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp eq " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                case BOp::ne: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp ne " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                case BOp::gt: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp sgt " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                case BOp::lt: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp slt " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                case BOp::ge: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp sge " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                case BOp::le: {
                                    const std::string cn = sname + "_cmp";
                                    const std::string rhs_bare = emitValueOperand(bin.rhs, program);
                                    output << "  " << cn << " = icmp sle " << lhs_typed << ", " << rhs_bare << "\n";
                                    output << "  " << llvmValueName(sname) << " = zext i1 " << cn << " to i32\n";
                                    break;
                                }
                                }
                            ...
                            [&](const yesod::Ref<koopa_ir::CallExpr>& callRef) {
                                ...
                                const auto sigIt = sigs.find(call.callee.spelling);
                                const std::string callRetTy = (sigIt != sigs.end()) ? sigIt->second.retType : "void";
                                // Mark the value type for the result of the call.
                                if (callRetTy != "void") {
                                    valueTypes[sname] = callRetTy;
                                }
                                output << "  " << llvmValueName(sname) << " = call " << callRetTy << " @"
                                       << callee << "(";
                                for (size_t ai = 0; ai < call.args.size(); ++ai) {
                                    if (ai != 0) { output << ", "; }
                                    const std::string argType = (sigIt != sigs.end() && ai < sigIt->second.paramTypes.size())
                                        ? sigIt->second.paramTypes[ai] : "i32";
                                    output << argType << " " << emitValueOperand(call.args[ai], program);
                                }
                                output << ")\n";
                            });
                    } else if constexpr (std::same_as<StmtNode, koopa_ir::StoreStmt>) {
                        const auto& store = program[stmtRef];
                        const auto ptIt = pointees.find(store.destination.spelling);
                        const std::string elemTy = (ptIt != pointees.end()) ? ptIt->second : "i32";
                        output << "  store " << elemTy << " ";
                        MATCH(store.value) WITH(
                            [&](const koopa_ir::Symbol& sv) { output << llvmValueName(sv.spelling); },
                            [&](const koopa_ir::IntegerLiteral& sv) { output << sv.value; },
                            [&](const koopa_ir::UndefValue&) { output << "undef"; },
                            [&](const koopa_ir::ZeroInit&) { output << "zeroinitializer"; },
                            [&](const yesod::Ref<koopa_ir::AggregateInitializer>&) { output << "zeroinitializer"; });
                        output << ", ptr " << llvmValueName(store.destination.spelling) << "\n";
                    } else if constexpr (std::same_as<StmtNode, koopa_ir::CallExpr>) {
        ...
        MATCH(block.terminator) WITH(
            [&](const yesod::Ref<koopa_ir::JumpTerminator>& jumpRef) {
                const auto& jump = program[jumpRef];
                output << "  br label %" << blockLabels[jump.target.spelling] << "\n";
            },
            [&](const yesod::Ref<koopa_ir::BranchTerminator>& brRef) {
                const auto& br = program[brRef];
                const std::string condName = "_cond_bool_" + stripPrefix(block.label.spelling);
                output << "  %" << condName << " = icmp ne i32 "
                       << emitValueOperand(br.condition, program) << ", 0\n";
                output << "  br i1 %" << condName << ", label %"
                       << blockLabels[br.trueTarget.spelling]
                       << ", label %" << blockLabels[br.falseTarget.spelling] << "\n";
            },
            [&](const yesod::Ref<koopa_ir::ReturnTerminator>& retRef) {
                const auto& ret = program[retRef];
                if (ret.value.has_value()) {
                    output << "  ret i32 " << emitValueOperand(*ret.value, program) << "\n";
                } else {
                    output << "  ret void\n";
                }
            });
