...
private:
    ...
    void generateBlock(
        Ptr<frontend::Block> body, FunctionGenerationState& state) const;
    void generateBlockItem(
        const frontend::BlockItem& blockItem, FunctionGenerationState& state) const;
    void generateDecl(
        frontend::Decl declNode, FunctionGenerationState& state) const;
    void generateStmt(
        frontend::Stmt stmtNode, FunctionGenerationState& state) const;
    void generateIfStmt(
        Ptr<frontend::IfStmt> ifStmt, FunctionGenerationState& state) const;
    ...
    void generateExpStmt(
        Ptr<frontend::ExpStmt> expStmt, FunctionGenerationState& state) const;
    [[nodiscard]] ReturnValue* generateReturnStmt(
        Ptr<frontend::ReturnStmt> returnStmt,
        FunctionGenerationState& state) const;
    [[nodiscard]] Value* generateExp(
        Ref<frontend::Exp> exp, FunctionGenerationState& state) const;
    ...
    [[nodiscard]] Value* generateBooleanAsInt(
        Ref<frontend::Exp> exp, FunctionGenerationState& state) const;
    void generateBooleanBranch(Ref<frontend::Exp> exp, BasicBlock& trueBlock,
    ...
    [[nodiscard]] Value* generateNumber(
        const frontend::Exp::Number& number) const;
    ...
    [[nodiscard]] const frontend::SemanticSymbol& requireSymbolForIdentifier(
        Ref<frontend::Identifier> identifier,
