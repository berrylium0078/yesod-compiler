...
struct ParserFunctionTest : ParserTestBase {
    ...
    {
        const auto declNode_nn = match(topLevelItemNode_nn(ast()).m_topLevelItem,
            with {
    ...
    void testGlobalsFunctionsAndCallsParse()
    {
        ...
        const auto addFunc_nn = requireTopLevelFuncDef(root()(ast()).m_topLevelItems[2]);
        ...
        require(addFunc_nn(ast()).m_funcFParams[0](ast()).m_identifier_nn(ast()).m_name
                == "lhs",
            "first formal parameter should preserve its identifier text");
        require(addFunc_nn(ast()).m_funcFParams[1](ast()).m_identifier_nn(ast()).m_name
                == "rhs",
            "second formal parameter should preserve its identifier text");

        const auto noopFunc_nn = requireTopLevelFuncDef(root()(ast()).m_topLevelItems[3]);
        ...
        const auto mainFunc_nn = requireTopLevelFuncDef(root()(ast()).m_topLevelItems[4]);
        const auto returnStmt_nn = extractReturnStmt(
            mainFunc_nn(ast()).m_block_nn(ast()).m_blockItems.front());
        const auto& callExp = requireCallExpHandle(returnStmt_nn(ast()).m_exp_nn);
