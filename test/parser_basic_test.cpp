...
void testCommentsAreSkippedBetweenTokens() {
    const std::string source =
        "/* leading block */ int /* name gap */ main // line comment before parens\n"
...
void testMalformedInputsFailWithFocusedDiagnostics() {
    ...
    require(!missingSemicolon.success(), "missing semicolon should produce a recovery diagnostic");
    require(missingSemicolon.m_root != nullptr, "missing semicolon should recover to the block boundary and still build the root");
    require(firstDiagnostic(missingSemicolon).m_kind == DiagnosticKind::missingSemicolon, "missing semicolon should report the PEG recovery label");
    require(firstDiagnostic(missingSemicolon).m_offset == 19, "missing semicolon should diagnose at the statement boundary before the closing brace");

    const auto malformedHex = parseSource("int main(){return 0x;}");
    require(!malformedHex.success(), "malformed hexadecimal literal should fail");
    require(firstDiagnostic(malformedHex).m_kind == DiagnosticKind::malformedReturnValue, "malformed hexadecimal literal should report the committed malformed-return diagnostic");

    const auto missingBrace = parseSource("int main(){return 1;");
    require(!missingBrace.success(), "missing closing brace should produce a recovery diagnostic");
    ...
    require(firstDiagnostic(malformedStmtHead).m_kind == DiagnosticKind::malformedStmtHead, "malformed statement head should report the PEG recovery label");

    const auto malformedReturnValue = parseSource("int main(){return ;}");
    require(!malformedReturnValue.success(), "missing return value should fail");
    require(firstDiagnostic(malformedReturnValue).m_kind == DiagnosticKind::malformedReturnValue, "missing return value should report the PEG recovery label");

    const auto trailingInput = parseSource("int main(){return 1;} trailing");
    require(!trailingInput.success(), "trailing input should fail");
    require(firstDiagnostic(trailingInput).m_kind == DiagnosticKind::trailingInput, "trailing input should produce a trailing-input diagnostic");
}
...
void testOutOfRangeIntegerFails() {
    ...
    require(firstDiagnostic(output).m_kind == DiagnosticKind::integerOutOfRange, "out-of-range integer should report range overflow");
}
