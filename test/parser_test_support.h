...
static_assert(std::is_same_v<decltype(std::declval<ReturnStmt>().m_exp_nn), std::shared_ptr<Exp>>);
...
inline const UnaryExp& requireUnaryExp(const std::shared_ptr<UnaryExp>& unaryExp_nn) {
    require(unaryExp_nn != nullptr, "expected unary expression node");
    return *unaryExp_nn;
}

inline const PrimaryExp& requirePrimaryExp(const std::shared_ptr<PrimaryExp>& primaryExp_nn) {
    require(primaryExp_nn != nullptr, "expected primary expression node");
    return *primaryExp_nn;
}

inline std::shared_ptr<ReturnStmt> extractReturnStmt(const std::shared_ptr<StmtNode>& stmtNode_nn) {
...
inline int32_t evaluateMulExp(const MulExp& mulExp) {
    ...
    for (const auto& tailEntry : mulExp.m_tail) {
        const auto rhsValue = evaluateUnaryExp(requireUnaryExp(tailEntry.second));
        switch (tailEntry.first) {
    ...
    return std::visit(
        [&](const auto& unaryAlt) -> int32_t {
            ...
            if constexpr (std::is_same_v<AltType, std::shared_ptr<PrimaryExp>>) {
                ...
                const auto operandValue = evaluateUnaryExp(requireUnaryExp(unaryAlt.second));
    ...
    return std::visit(
        [&](const auto& unaryAlt) -> bool {
            ...
            if constexpr (std::is_same_v<AltType, std::shared_ptr<PrimaryExp>>) {
                ...
                return containsParenthesizedUnary(requireUnaryExp(unaryAlt.second));
            }
        },
        unaryExp.m_kind);
}
