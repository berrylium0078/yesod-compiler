...
struct ParserWhileTest : ParserTestBase {
    void testWhileLoopParsesWithLoopControlStatements()
    {
        ...
        require(evaluateExp(whileStmt_nn(ast()).condition) == 1,
            "while condition should reuse expression parsing");

        const auto bodyBlock_nn = extractBlockStmt(whileStmt_nn(ast()).body);
        require(bodyBlock_nn(ast()).items.size() == 2,
            "while body block should preserve both loop-control statements");
        require(extractContinueStmt(
                    extractStmtNode(bodyBlock_nn(ast()).items[0]))
                != nullptr,
            "while body should preserve continue statements");
        require(extractBreakStmt(
                    extractStmtNode(bodyBlock_nn(ast()).items[1]))
                != nullptr,
    ...
    void testNestedWhileLoopParsesInnerLoopControlStatements()
    {
        ...
        const auto innerWhile_nn = extractWhileStmt(outerBody_nn(ast()).items[0]);
        const auto innerBody_nn = extractBlockStmt(innerWhile_nn(ast()).body);

        require(innerBody_nn(ast()).items.size() == 2,
    ...
    void testLoopControlInsideWhileIfParses()
    {
        ...
        require(evaluateExp(ifStmt_nn(ast()).condition) == 2,
            "while-if body should preserve the nested if condition");
        require(extractBreakStmt(ifStmt_nn(ast()).thenBody) != nullptr,
            "while-if then branch should preserve break statements");
        require(extractContinueStmt(ifStmt_nn(ast()).elseBody) != nullptr,
    ...
    void testBreakAndContinueRecoveryMakeForwardProgress()
    {
        ...
        require(firstFuncDef()(ast()).body(ast()).items.size()
                == 2,
