...
struct ParserPolyTest : ParserTestBase {
    ...
    void testPolyVarDeclAndCast()
    {
        ...
        const auto& mainFunc = *std::get_if<Ref<FuncDef>>(
            &root()(ast()).topLevelItems[0]);
        ...
        require(initExp2 != nullptr, "second poly initializer should be scalar");
    ...
    void testPolyBang()
    {
        ...
        require(unary.op == UnaryOpKeyword::bang,
            "!poly should parse as bang unary operator");
        const auto& lVal = requireLVal(unary.lhs);
        require(lVal.indices.empty(),
            "!poly lval should have no indices");
    }
    ...
    void testPolyCoeffExtract()
    {
        ...
        require(index.value == 2,
            "poly[2] subscript index should be 2");
    }
    ...
    void testPolyShift()
    {
        ...
        require(lhsLVal.indices.empty(),
            "shift lhs should be simple lval");
        const auto& rhsNumber = requireNumber(binary.rhs);
        require(rhsNumber.value == 2,
            "shift rhs should be 2");
    }
    ...
    void testPolySliceWithVars()
    {
        ...
        require(baseLVal.identifier(ast()).name == "p",
            "slice base should be p");
        // Start should be LVal 'n'
        const auto& startLVal = requireLVal(slice.start);
        require(startLVal.identifier(ast()).name == "n",
            "slice start should be n");
        // End should be LVal 'm'
        const auto& endLVal = requireLVal(slice.end);
        require(endLVal.identifier(ast()).name == "m",
            "slice end should be m");
    }
    ...
    void testPolyCastMissingRParen()
    {
        ...
        require(!success(),
            "poly cast missing ')' should fail");
        require(isDiagnostic<MissingCastRParenDiagnostic>(firstDiagnostic()),
            "missing ')' after poly cast should report cast delimiter diagnostic");
    }
    ...
    void testPolySliceMissingBracket()
    {
        ...
        require(!success(),
            "malformed slice should fail parsing");
    }
