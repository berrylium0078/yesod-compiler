...
- **切片**：`p[n, m]` 切片操作，结果只有 `[n,m)` 范围内的系数非零
- **移位**：`p << k` / `p >> k` 移位操作，负数会截断（相当于结果对 `[0,+inf)` 切片）
- **显式构造**：`poly(int)` / `poly(mint)` 构造仅含常数项的多项式

多项式没有别名，两个不同多项式对象底层数组不同。
...
- **Combine 融合算子生成**：线性多项式表达式生成融合算子，将加减、数乘、移位、切片整理为“先切片再移位的线性组合”
- **Pointwise 融合算子生成**：可点值化的表达式和多项式乘法生成 `pointwise`，在 NTT 域中逐点计算表达式树
...
融合算子会贪心合并，但一般不会跨越表达式，因为难以建立合适的代价模型。这样设计多数时候较优，特殊情况可以让用户手动提取公共子表达式。

还有一个问题是，当前对“多项式的点值表示”的复用主要依赖后续 CSE，但复杂情况会复用失败。未来工作可以尝试添加点值表示的多项式类型，让用户选择何时复用。
...
5. **多项式运行时调用生成**
   ...
   - 函数调用会将 poly 类型参数所有权移交被调用者，因此需要为仍活跃的参数进行硬复制（包括重复传参）。多项式类型返回值所有权移交给调用者
   - edge plan 代码生成；特别地，branch 需要拆出中间基本块
...
- **Arena 分配器**：所有 AST 和 Koopa IR 节点均通过分配器统一管理，使用类型安全的 `Ref<T>`（非空引用）和 `Ptr<T>`（可为空指针）句柄。这消除了手动内存管理的负担，确保确定性释放且无内存泄漏；而且 32 位句柄比 64 位指针短，可以提高缓存命中率。
- **C++17 STL**：使用 `std::optional` 等模板类实现数据结构，`std::visit` 提供类型安全、穷尽检查的变体匹配。
- **PEG 解析器**：采用 PEG 文法，因为课上讲了想试一试。
- **多阶段语义分析**：语义分析分为符号解析、类型分析、CFG 构建、SSA 构建四个独立子阶段，每个阶段可以单独测试，降低了整体复杂度。
- **Koopa IR 作为桥梁**：自定义的 Koopa IR 起到了前端与后端之间的桥梁作用，它同时具备高层语言语义（如 `mint`、`poly` 类型）和底层控制流表示（基本块、SSA），使得优化 Pass 可以在一个统一的 IR 上工作；此外每个算子的 LLVM IR 代码生成逻辑可以独立实现，便于维护和扩展。

### 3.3 项目实现过程中的经验

1. **带错误恢复的 PEG 解析器**：引入了带有错误 token 同步的错误恢复机制，提升了用户体验，而且可以减少 PEG 记忆化搜索的内存消耗。

2. **SSA 构建**：支配树计算（朴素实现）和基于活跃分析和支配边界的基本块参数插入。

3. CSE 和 DCE

4. **多项式类型的内存管理**：`poly` 类型在堆上分配系数数组，其生命周期管理是巨大的挑战。采用 SSA 形式活跃性分析来确定释放点，避免了引用计数和 GC 的开销，而且可以自动 move 或复用数组（例子可参考示例代码的读入和 `set_coeff` 函数），但问题是可能导致代码膨胀（例如一个巨大的 if-else 结构，只有一个分支使用，其他每个分支都要释放一次；虽然可以保证 drop 幂等并改成在汇合点（或作用域结束时）统一释放，但是我觉得更激进的释放可能会提高缓存命中（？），而且我对 poly 类型也做了覆写传播导致需要在基本块跳转前复制重复的基本块参数，所以最终实现没有这样做）。总的来说是一次有趣的尝试。

5. 其实最开始类型扩展想通过自举实现辅助函数的编译，但问题是 mint 乘法中间结果需要 int64 类型。最后解决办法是直接用 clang -O2 编译然后链接或者类似 `#include` 导入。

6. **测试驱动开发**：项目始终坚持先写测试再实现的策略，每个特性都有对应的单元测试，此外还实现了解释器（`interpreter.cpp`）作为参考实现，确保编译器生成的代码与解释器行为一致。在编译器测试中，程序行为断言很方便实现和迁移。
...
3. 提示词工程练习：我试着手搓了一套可复用的提示词，让 AI 在实现解析器前先生成一个完整的 PEG 文法描述和 AST 数据结构的关键决策，之后再生成解析器代码。
...
我删除了 lvX 中的部分性能测试，因为这些测试是用来确保编译器正确性的。
...
（我们的 Montgomery reduction 没有优于编译器的 Barrett reduction 的原因有待探究……）
...
- **类型安全**：`Ref<T>` / `Ptr<T>` 区分非空和可空引用，`std::variant` + `std::visit` 穷尽检查
